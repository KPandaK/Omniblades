...
sass.render({
	...
	if (!error) {
		...
		fs.writeFile("omniblades.css", cssOutput, printOutput);
	} else {
...
// Build HTML
const htmlOutput = pug.renderFile("Source/Omniblades.pug", options).trim().replace(/\n+/g, "\n");
fs.writeFile("omniblades.html", `${htmlOutput}\n`, printOutput);
